...
def reward(sample_solution, use_cuda=True, name='reward'):
    ...
    if 'CH' in name:
        ...
        print "vec_area vec_area vec_area"
        print vec_area.mean()
...
class GraphEmbedding(nn.Module):
    def __init__(self, input_size, ebd_size, use_cuda=True, use_sdne=True, add_noise=False, is_training=True):
        ...
        print ebd_size_1,ebd_size_2,ebd_size_3
...
class PointerNet(nn.Module):
    def __init__(self,
        ...
        #print "ebd_size ebd_size ebd_size"
        #print ebd_size

        #print "hidden_size hidden_size hidden_size"
        #print ebd_size
    ...
    def forward(self, inputs, is_training, add_noise):
        ...
        #print "embedded embedded embedded"
        ...
        for i in range(seq_len):
            ...
            #print "encoder_outputs encoder_outputs encoder_outputs"
...
class TrainModel:
    ...
    def train_and_validate(self, args, n_epochs):
        ...
        for epoch in range(self.start_epoch, n_epochs):
            ...
            for batch_id, sample_batch in data_loaders:
                ...
                if batch_id == 0:
                    ...
                    #critic_exp_mvg_avg = (critic_exp_mvg_avg * args.beta) + ((1. - args.beta) * R.mean())
                     critic_exp_mvg_avg = (critic_exp_mvg_avg * args.beta) + ((1.001 - args.beta) * R.mean())
                elif args.use_decay and epoch >= 5:
                ...
                print "critic_exp_mvg_avg critic_exp_mvg_avg critic_exp_mvg_avg"

                print critic_exp_mvg_avg
                ...
                self.train_area.append(R.mean().detach())
                ...
                if batch_id % 20 == 0:
                    ...
                    for val_batch in self.val_loader:
                        ...
                        self.val_area.append(R.mean().item())
            ...
            model_name = "Convex Hull_%d" % self.epochs
            self.save_model(save_dir, model_name)

            if self.threshold and self.train_area[-1] < self.threshold:
        ...
        print("\nTraining Completes!",
              ...
              "Final training reward: %4.4f" % self.train_area[-1],
              "Final validation reward: %4.4f" % self.val_area[-1])

    def plot(self, args, epoch, batch_id):
        if 'CH' in args.problem_name:
            clear_output(True)
            plt.figure(figsize=(12, 12))
            plt.ion()
            plt.subplot(221)
            plt.title('train tour area: epoch %d batch_id %d reward %s' %
                      (epoch, batch_id, '%4.2f' % self.train_area[-1] if len(self.train_area) else 'collecting'))
            plt.plot(self.train_area, color='blue', linestyle='-')
            plt.plot(self.critic_exp, color='pink', linestyle='-')
            plt.grid()
            plt.subplot(222)
            plt.title('val tour area: epoch %d batch_id %d reward %s' %
                      (epoch, batch_id, '%4.2f' % self.val_area[-1] if len(self.val_area) else 'collecting'))
            plt.plot(self.val_area)
            plt.grid()
            plt.subplot(223)
